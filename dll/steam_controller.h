...
// ISteamController v008 and ISteamInput both declare GetActionOriginFromXboxOrigin with
// identical parameter types but different return types (EControllerActionOrigin vs
// EInputActionOrigin), so a single C++ class cannot implement both. The two enums share
// identical values, so at the ABI level the vtable slot is interchangeable. This shim is
// method-for-method identical to ISteamController (v008) except that the clashing method
// keeps the historical trailing-underscore name (as in ISteamController007).
class ISteamController_v008_shim
{
public:
...
public ISteamController_v008_shim,
